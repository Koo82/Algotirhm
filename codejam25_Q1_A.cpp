...
int main() {
    ...
    while(t--) {
        cin >> n;
        arr = vector(110, vector<set<int>>(110));
        check = vvb(110, vb(110, false));
        rep(k, n) {
            cin >> o >> d >> y >> x;
            if(o==1) {
                int cnt1 = -1;
        ...
        rep(i, 110) {
            rep(j, 110) {
                if(check[i][j]) ans += 1;
                else if(sz(arr[i][j])==1) ans += 0.5;
                else if(sz(arr[i][j])==2) {
                    int tot = 0;
                    for(auto val: arr[i][j]) tot += val;
                    if(tot == 5) ans += 1;
                    else ans += 0.75;
                }
                else if(sz(arr[i][j])>=3) ans += 1.0;
            }
        }
