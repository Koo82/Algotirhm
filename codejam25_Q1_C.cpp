...
vector<vvl> arr;
vl dist;
// vti com = {{0,1,2}, {1,0,3}, {2,0,3}, {3,1,2}};
vti com = {{3,1,2}, {2,0,3}, {1,0,3}, {0,1,2}}; // 작게 만들어야 할 원소로 정렬



pl calc(ll kk , ll aa, ll bb, ll cc, ll dd) {
    if(kk==1) {
        if(aa==1 && bb==0 && cc==0 && dd==0) return {1,1};
        else if(aa==0 && bb==1 && cc==0 && dd==0) return {1,0};
        else if(aa==0 && bb==0 && cc==1 && dd==0) return {0,1};
        else if(aa==0 && bb==0 && cc==0 && dd==1) return {0,0};           
        else return {INF,INF};

    }
    // ll x = -1, y = -1; 
    ll a=0, b= 0, c = 0, d = 0, ta =0, tb=0, tc=0, td=0;
    
    ll idx  = min({aa, bb, cc, dd});
    vi candi;
    if(idx == aa) candi.push_back(0);
    if(idx == bb) candi.push_back(1);
    if(idx == cc) candi.push_back(2);
    if(idx == dd) candi.push_back(3);
    for(auto idx:candi) {
        auto [i, j, k] = com[idx];
        a = 2*arr[kk-1][i][0] + arr[kk-1][j][0] + arr[kk-1][k][0] + (idx==3 ? 1 : 0);
        b = 2*arr[kk-1][i][1] + arr[kk-1][j][1] + arr[kk-1][k][1] + (idx==2 ? 1 : 0);
        c = 2*arr[kk-1][i][2] + arr[kk-1][j][2] + arr[kk-1][k][2] + (idx==1 ? 1 : 0);
        d = 2*arr[kk-1][i][3] + arr[kk-1][j][3] + arr[kk-1][k][3] + (idx==0 ? 1 : 0);
        
        ta = arr[kk-1][i][0] + arr[kk-1][j][0] + arr[kk-1][k][0] + (idx==3 ? 1 : 0);
        tb = arr[kk-1][i][1] + arr[kk-1][j][1] + arr[kk-1][k][1] + (idx==2 ? 1 : 0);
        tc = arr[kk-1][i][2] + arr[kk-1][j][2] + arr[kk-1][k][2] + (idx==1 ? 1 : 0);
        td = arr[kk-1][i][3] + arr[kk-1][j][3] + arr[kk-1][k][3] + (idx==0 ? 1 : 0);
        int dir;
        for(dir=0; dir<4; ++dir) {
            if(dir!=i && dir!=j && dir!=k) break;
        }
        if((aa+bb+cc+dd==a+b+c+d) && aa>=ta && bb>=tb && cc>=tc && dd>=td) {
            pl loc;
            if(dir==0) loc = {0, 0}; 
            else if(dir==1) loc = {0, dist[kk]};
            else if(dir==2) loc = {dist[kk], 0};
            else loc = {dist[kk], dist[kk]}; // (dir==3)
            // int naa = 
            pl ans = calc(kk-1, aa-ta, bb-tb, cc-tc, dd-td);
            if(ans.first<INF && ans.second<INF) return {loc.first+ans.first, loc.second+ans.second}; 
        }
    }
    return {INF, INF};
}
...
int main() {
    fastio;
    arr = vector(31, vvl(4, vl(4))); // a, b, c, d, 1,2,3,4 분면;
    arr[1][0] = {1, 0, 0, 0}, arr[1][1] = {0, 1, 0, 0}, arr[1][2] = {0, 0, 1, 0}, arr[1][3] = {0, 0, 0, 1};
    loop(i, 2, 31) {
        arr[i][0][0] = 1, arr[i][1][1] = 1, arr[i][2][2] = 1, arr[i][3][3] = 1;
        rep(j, 4) { // 0는 A파트, 1는 B파트, 2는 C파트, 3은 D파트
            arr[i][0][j] += (ll)2 * arr[i-1][0][j] + arr[i-1][1][j] + arr[i-1][2][j];
            arr[i][1][j] += arr[i-1][0][j] + (ll)2 * arr[i-1][1][j] + arr[i-1][3][j];
            arr[i][2][j] += arr[i-1][0][j] + (ll)2 * arr[i-1][2][j] + arr[i-1][3][j];
            arr[i][3][j] += arr[i-1][1][j] + arr[i-1][2][j] + (ll)2 * arr[i-1][3][j];
        } 
    }
    dist = vl(35);
    dist[1] = 1;
    ...
    while(t--) {
        ...
        // int idx = 29;
        // rep(i, 4) {
        //     // tt[i] = arr[idx][0][i]+arr[idx][1][i]+arr[idx][2][i]+arr[idx][3][i];
        //     tt[i] = (ll)2*arr[idx][0][i]+arr[idx][1][i]+arr[idx][2][i]; // +arr[idx][3][i];
        //     cout << tt[i] << '\n';
        // }
        pl ans = calc(k, a, b, c, d);
        if(ans.first>=INF || ans.second>=INF) cout << "-1 -1\n";
