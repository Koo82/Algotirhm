...
int main() {
    fastio;
    cin >> n >> k;
    // arr = vpi(n), cost = vpi(n);
    arr = vector<pair<pl, pl>>(n);
    map<int, tl> resi; // , dec; // resi -> residual 제거, dec는 mul에서 계수를 빼주는 시기를 저장
    ll cur = 0, mul = 0, cnt = 0;
    set<int> contain;
    rep(i, n) {
        int s, e, p, d; cin >> s >> e >> p >> d;   
        arr[i] = {{s, e}, {p, d}};
    }
    asort(arr);

    rep(i, n) {
        auto [peri, info] = arr[i];
        auto [s, e] = peri;
        // auto [p, d] = info;
        if(s-1 != arr[0].first.first-1 && !contain.count(s-1)) {
            arr.push_back({{s-1, INF}, {0, 0}});
            contain.insert(s-1);
        }
        if(!contain.count(s)) {
            arr.push_back({{s, INF}, {0, 0}});
            contain.insert(s);
        }
        if(!contain.count(e)) {
            arr.push_back({{e, INF}, {0, 0}});
            contain.insert(e);
        }
    }
    asort(arr);
    ll idx = arr[0].first.first;
    rep(i, sz(arr)) {
        auto [peri, info] = arr[i];
        auto [s, e] = peri;
        auto [p, d] = info;

        while(!resi.empty() && resi.begin()->first <= s) {
            auto [r, de, c] = resi.begin()->second;
            
            cnt -= c;
            cur -= r;
            mul -= de;
            resi.erase(resi.begin());
        }
        ...
        if(e!= INF) {
            ...
            ++cnt;
            if(resi.count(e+1)) {
                auto& [r, de, c] = resi[e+1];
                r += p-(e-s)*d;
                de += d;
                ++c;
            }
            else resi[e+1] = {p-(e-s)*d, d, 1};
            // resi[e+1] = {p-(e-s)*d, d};
        }

        if(e==INF && cnt >=k) ans = min(ans, cur);
    }
    if(ans==LINF) cout << "-1";
    else cout << ans;
